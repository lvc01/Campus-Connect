...
function StatCard({ label, value, color }: { label: string; value: number | string; color: string }) {
  return (
    <div className="bg-bg-surface border border-border rounded-xl p-4 flex flex-col gap-1">
      <p className="text-xs font-bold tracking-widest text-text-muted uppercase">{label}</p>
    ...
    <div className="flex flex-col gap-2">
      {entries.map(([key, count]) => (
        <div key={key} className="flex items-center gap-3">
          <span className="text-xs font-semibold text-text-main w-24 shrink-0">{labels[key] || key}</span>
          ...
          <span className="text-xs font-bold text-text-muted w-8 text-right">{count}</span>
        </div>
      ))}
    </div>
  );
...
export function ModerationAnalytics() {
    ...
    return (
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 animate-pulse">
        {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
          <div key={i} className="bg-bg-surface border border-border rounded-xl p-4 h-20" />
        ))}
      </div>
    );
  }
  ...
  if (!stats) {
    return <p className="text-sm text-text-muted">Failed to load stats.</p>;
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        <StatCard label="Total" value={stats.total_reports} color="text-text-main" />
        ...
        <StatCard label="Dismissed" value={stats.dismissed} color="text-text-muted" />
      </div>
      ...
      <div className="bg-bg-surface border border-border rounded-xl p-5 flex items-center justify-between">
        <div>
          <p className="text-xs font-bold tracking-widest text-text-muted uppercase">Resolved Today</p>
          <p className="text-3xl font-black text-emerald-400 mt-1">{stats.resolved_today}</p>
        </div>
        {stats.pending_oldest && (
          <div className="text-right">
            <p className="text-xs font-bold tracking-widest text-text-muted uppercase">Oldest Pending</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-bg-surface border border-border rounded-xl p-5">
          <h3 className="text-xs font-bold tracking-widest text-text-muted uppercase mb-4">By Category</h3>
          <BreakdownList data={stats.by_category} labels={CATEGORY_LABELS} />
        </div>
        <div className="bg-bg-surface border border-border rounded-xl p-5">
          <h3 className="text-xs font-bold tracking-widest text-text-muted uppercase mb-4">By Target Type</h3>
      ...
      {/* Priority breakdown */}
      <div className="bg-bg-surface border border-border rounded-xl p-5">
        <h3 className="text-xs font-bold tracking-widest text-text-muted uppercase mb-4">By Priority</h3>
